    ...
    <v-flex xs12>
      <v-tabs centered icons-and-text>
        ...
        <v-tab-item value="tab-1">
          <v-container>
            <v-switch
              @change="sendVisualsConfig('glow')"
              color="primary"
              v-model="glow"
              label="Enable/disable glow outline"
            />
          </v-container>
        </v-tab-item>
        <v-tab-item value="tab-2">
          <v-container>
            <v-switch
              @change="sendVisualsConfig('radar')"
              color="primary"
              v-model="radar"
              label="Enable/disable Enemies spotted on radar"
            />
          </v-container>
        </v-tab-item>
        <v-tab-item value="tab-3">
          <v-container>
            <v-switch
              @change="sendVisualsConfig('noFlash')"
              color="primary"
              v-model="noFlash"
              label="Enable/disable noFlash functionality"
            />
          </v-container>
        </v-tab-item>
      </v-tabs>

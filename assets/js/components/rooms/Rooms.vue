<template>
    <div>
        <v-snackbar :timeout="3000" bottom right class="success--text" v-model="RoomRemovedAlert">Raum erfolgreich gelöscht</v-snackbar>
        ...
        <v-dialog transition="slide-y-transition" persistent v-model="deleteDialog" max-width="500px">
            <v-card>
                <v-card-title>
                    <span class="title">Raum <b>{{ this.selectedRoom.title }}</b> löschen</span>
                </v-card-title>
                <v-card-text>
                    <p>Sind Sie sicher, dass Sie die Raum <b>{{ this.selectedRoom.title }}</b> mit der ID <b>{{ this.selectedRoom.id }}</b> löschen wollen?</p>
                </v-card-text>
                <v-divider></v-divider>
                <v-card-actions>
                    <v-btn color="error" flat @click="deleteDialog = !deleteDialog; loading = null">Abbrechen</v-btn>
                    <v-spacer></v-spacer>
                    <v-btn color="primary" flat @click="deleteDialog = !deleteDialog; deleteRoom(selectedRoom.id);">Löschen</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        <v-container v-if="mounted" fluid>
            ...
            <v-card>
                <v-card-title>
                    <h4>Liste aller Räume</h4>
                    <v-spacer></v-spacer>
                    <v-text-field append-icon="search" label="Räume durchsuchen" single-line hide-details v-model="search"></v-text-field>
                </v-card-title>
                <v-data-table
                ...
                >
                    <template slot="items" slot-scope="props">
                        ...
                        <td class="text-xs">{{ props.item.housing.title }}</td>
                        <td>
                        ...
                        <td>
                            <v-btn
                                ...
                                icon
                                class="mx-0"
                                @click="deleteDialog = true; selectedRoom = props.item; loading = props.item.id"
                            >
    ...
    data () {
        return {
            ...
            headers: [
                ...
                { text: 'Unterbringungsmöglichkeit', value: 'housing.title' },

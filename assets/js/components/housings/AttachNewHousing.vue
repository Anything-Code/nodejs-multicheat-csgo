<template>
    <div>
        <v-layout class="mt-3" align-center>
            ...
            <b class="primary--text cursor-pointer" @click="dialog = true">neue Unterbringungsmöglichkeit erstellen</b>
        </v-layout>
        <v-dialog v-model="dialog" fullscreen hide-overlay transition="fade-transition">
            <div v-if="dialog" class="bg-main" :class="$parent.isDark ? 'dark' : 'grey lighten-5'">
                ...
                <div class="pa-3">
                    <v-card v-if="mounted && housingCategoriesMounted">
                        <v-container fluid>
        ...
        storeHousing (event) {
            ...
            if (this.$refs.form.validate()) {
                ...
                { headers: { "Content-Type": "application/json", "X-Requested-With": "XMLHttpRequest", "Authorization": "Bearer " + this.$auth.getToken() } }
                    ...
                    this.housingName = '';
                    this.destination = '';
                    this.housingCategories = [];
                    this.editorContent = '';
                    this.attachedImages = [];
                    this.dialog = false;
                    this.mainLoading = false;
                    this.$emit('housingStored', response.data.data);
                })
                .catch(error => {
                    console.log(error);
                });
